...
class App:
    def __init__(self):
        ...
        self.clouds = [Cloud(self) for i in range(3)]
        ...
        self.reset_rect.center = (WIDTH // 2, HEIGHT // 2 + 50)
    ...
    def update(self):
        ...
        if len(self.obstacles) == 0:
            ...
            if rand_obs == 0:
                self.obstacles.append(SmallCactus(app, SMALL_CACTUS))
            elif rand_obs == 1:
                self.obstacles.append(LargeCactus(app, LARGE_CACTUS))
            else:
                self.obstacles.append(Bird(app, BIRD))
        ...
        for obs in self.obstacles:
            ...
            obs_x, obs_y = obs.getPos()

            offset_x, offset_y = obs_x - dino_x, obs_y - dino_y

            if self.dinosaur.getMask().overlap(obs.getMask(), (offset_x, offset_y)):
                self.death = True
    ...
    def run(self):
        while True:
            ...
            if self.user_input[pg.K_SPACE] and self.death:
                self.reset()
